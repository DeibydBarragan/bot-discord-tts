...
/**
 * @param {string} text
 * @param {Language} [lang="es-CO"]
 * @param {boolean} [slow=false]
 * @param {string} cfg.host
 * @param {number} cfg.timeout
 * @param {string} cfg.splitPunct
 */
function getVoiceStream(text, {lang, slow = false, host = 'https://translate.google.com', timeout = 10000, splitPunct} = {}) {
 ...
 * @param {string} text
 * @param {PlainObject} cfg
 * @param {Language} [cfg.lang="es-CO"]
 * @param {number} [cfg.slow=false]
 * @param {string} cfg.host
 * @param {number} cfg.timeout
 * @param {string} cfg.splitPunct
 */
function saveToFile(filePath, text, {lang, slow = false, host, timeout, splitPunct} = {}) {
    const stream = new Stream.PassThrough();
